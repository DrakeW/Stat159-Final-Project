...
To analyze factors that affect graduation rates, we generated another model. Surprisingly, Lasso eliminated all variables except \texttt{MN_EARN_WNE_P10} though the coefficient estimate for \texttt{MN_EARN_WNE_P10} is pretty close to 0. And OLS generated an oddly large estimate for \texttt{UGDS_AIAN}, which might result from outliers. Among the rest predictors, \texttt{UGDS_MEN}, which reflects male-to-female ratio, had a relatively large coefficient, which makes it a prominent predictor. \texttt{FIRST_GEN}, which calculates the share of students in which neither parent completed college, also affects the graduation rates. Other influential predicts include those related to ethnic ratio (\texttt{UGDS_*}) or degree profile such as \texttt{PREDDEG} as explained in the previous model.
